...
export class HttpApprovalStore implements ApprovalStore {
  ...
  constructor({ baseUrl, fetcher = globalThis.fetch, headers, credentials = "include" }: HttpApprovalStoreOptions) {
    if (!fetcher) {
      throw new Error("A fetch implementation is required to use HttpApprovalStore");
    }
  ...
  async list(input: ApprovalQueueFilter & { signal?: AbortSignal } = {}): Promise<ApprovalRecord[]> {
    ...
    return this.request<ApprovalRecord[]>(`/v0/approvals${query}`, { signal: input.signal });
  }

  async get(id: string, signal?: AbortSignal): Promise<ApprovalRecord | null> {
    const response = await this.fetcher(`${this.baseUrl}/v0/approvals/${encodeURIComponent(id)}`, {
      signal,
  ...
  async decide(id: string, decision: ApprovalDecision, signal?: AbortSignal): Promise<ApprovalRecord> {
    const result = await this.request<DecisionResult>(`/v0/approvals/${encodeURIComponent(id)}/decisions`, {
      method: "POST",
      signal,
  ...
  async audit(id: string, signal?: AbortSignal): Promise<AuditEvent[]> {
    return this.request<AuditEvent[]>(`/v0/approvals/${encodeURIComponent(id)}/audit`, { signal });
  }

  private async request<T>(path: string, init?: RequestInit): Promise<T> {
    const headers = new Headers(this.headers);
    new Headers(init?.headers).forEach((value, key) => headers.set(key, value));
    return this.read<T>(await this.fetcher(`${this.baseUrl}${path}`, {
      ...init,
